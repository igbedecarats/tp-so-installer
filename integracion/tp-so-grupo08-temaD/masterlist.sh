...
BINDIR=`grep '^BINDIR' $CONFIGURACION | sed 's-[^=]*=\([^=]*\)=.*-\1-'`
MAEDIR=`grep '^MAEDIR' $CONFIGURACION | sed 's-[^=]*=\([^=]*\)=.*-\1-'`
LISTAS="$GRUPO/$MAEDIR/listas"
PROCESADAS="$LISTAS/procesadas"
RECHDIR=`grep '^RECHDIR' $CONFIGURACION | sed 's-[^=]*=\([^=]*\)=.*-\1-'`
...
do
	...
	then
		...
		then
			motivoRechazo="Cantidad de campos invalida"
		...
		then
			...
			then
				...
				then
					...
					regCount1=`grep -c "$superId;$usuario;$fechaLista.*" $PRECIOSMAE`
					sed -i "/$superId;$usuario;$fechaLista.*/d" "$PRECIOSMAE"  # borro los registros con super+usuario+fechaLista 
					regCount2=`grep -c "$superId;$usuario;$fechaLista.*" $PRECIOSMAE`
					regsEliminados=$(($regCount1 - $regCount2))
					#echo "borrados: $regsEliminados"
				fi
			fi
	...
	then
		...
		$GRUPO/$BINDIR/mover.sh $LISTAS/$file $RECHDIR  # mueve el archivo a rechazados
	fi
done
